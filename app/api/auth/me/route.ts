...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        const user = await db.prepare('SELECT id, name, email, role, department, rubro, approved, modules, panel_access, cliente_asignado, sector_asignado, cedula FROM users WHERE id = ?').get(session.user.id);
