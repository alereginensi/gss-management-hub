...
export async function POST(
    ...
    try {
        ...
        // Verify user exists
        const user = db.prepare('SELECT id, name, email FROM users WHERE id = ?').get(userId) as { id: number; name: string; email: string } | undefined;
        if (!user) {
        ...
        const existing = db.prepare('SELECT id FROM ticket_collaborators WHERE ticket_id = ? AND user_id = ?').get(ticketId, userId);
        if (existing) {
...
export async function DELETE(
    ...
    try {
        ...
        // Remove collaborator
        const result = db.prepare('DELETE FROM ticket_collaborators WHERE ticket_id = ? AND user_id = ?').run(ticketId, userId);
