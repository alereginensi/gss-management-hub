...
export async function GET(request: Request) {
    ...
    try {
        ...
        // 1. Status Filter
        if (filter === 'Abiertos') {
            conditions.push("status IN ('Nuevo', 'En Progreso')");
        } else if (filter === 'Cerrados') {
            conditions.push("status = 'Resuelto'");
        }
        ...
        if (session.user.role === 'admin' && departmentFilter !== 'Todos') {
            ...
            // Supervisors see their department or assigned to them
            // Logic is complex to replicate exactly in SQL for one simple export unless we duplicate strict logic.
            // For export, let's simplify: Supervisors see ALL tickets of their department.
