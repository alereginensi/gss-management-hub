...
export default function LimpiezaSolicitudesUniformePage() {
  ...
  useEffect(() => {
    ...
    if (currentUser && !hasModuleAccess(currentUser, 'limpieza')) { router.push('/'); return; }
  }, [loading, isAuthenticated, currentUser, router]);

  const fetchRows = useCallback(async () => {
    setFetching(true);
    try {
      const res = await fetch('/api/logistica/agenda/requests?source=limpieza');
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#f5f7fa' }}>
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <Link href="/operaciones-limpieza" style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', color: 'rgba(255,255,255,0.7)', textDecoration: 'none', fontSize: '0.78rem' }}>
            <ArrowLeft size={13} />{!isMobile && ' Limpieza'}
          </Link>
