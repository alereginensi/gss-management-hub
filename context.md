...
- **Worker service** (`agenda-worker`): segundo proceso separado que corre `node scripts/cron-agenda.cjs` (start command `npm run agenda:cron`). Dos schedules tz `America/Montevideo`: `0 9 28 * *` (genera slots del mes siguiente, default martes/jueves) y `0 2 * * *` (sync de renovaciones — re-habilita empleados con artículos vencidos). Expone `/api/health`. **Arquitectura**: worker ligero que sólo hace `node-cron` + `fetch()`; dispara los jobs vía HTTP al servicio principal en endpoints internos `POST /api/internal/agenda-cron/generate-slots` y `/sync-renewals`, protegidos con header `X-Cron-Secret` (env var `CRON_SECRET` compartida). Así no se duplica lógica SQL ni hace falta empaquetar `pg` + sub-deps en el worker. Env vars del worker: `INTERNAL_APP_URL` (URL privada del principal) + `CRON_SECRET`. Ver [docs/RAILWAY_WORKERS.md](docs/RAILWAY_WORKERS.md) para los pasos exactos de alta.
