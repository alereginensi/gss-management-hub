...
async function downloadMitrabajo(targetDate, options = {}) {
  ...
  // En Railway/Linux usar chromium del sistema (nix) para evitar conflictos de
  // home directory entre el usuario de build y el de runtime
  let executablePath;
  if (!debug) {
    try {
      ...
      if (found) { executablePath = found; console.log(`[mitrabajo] Chromium del sistema: ${found}`); }
    } catch { /* usar el de playwright en dev local */ }
  }
