...
async function downloadMitrabajo(targetDate, options = {}) {
  ...
  try {
    ...
    // 3. Esperar inputs de fecha (a veces cargan después del HTML inicial)
    const fechaDesdeSel = '#monitor-fechadesde';
    const fechaHastaSel = '#monitor-fechahasta';
    console.log(`[mitrabajo] Esperando campos de fecha en ${page.url()}...`);
    await page.waitForSelector(fechaDesdeSel, { state: 'visible', timeout: NAVIGATION_TIMEOUT_MS });
    await page.waitForSelector(fechaHastaSel, { state: 'visible', timeout: NAVIGATION_TIMEOUT_MS });

    // 4. Setear fechaDesde y fechaHasta
    console.log(`[mitrabajo] Seteando filtro a ${fechaSite}...`);
    for (const fieldId of [fechaDesdeSel, fechaHastaSel]) {
