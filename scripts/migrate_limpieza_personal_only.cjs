...
async function main() {
    ...
    for (const row of rawRows) {
        ...
        if (!c) {
            console.warn('⚠️  Fila omitida (cédula vacía), id sqlite:', row.id);
            continue;
        }
        byCedula.set(c, row);
    }
    ...
    try {
        ...
        if (hasLb.rows[0].e && hasLbc.rows[0].e) {
            ...
            console.warn('⚠️  No hay tablas logbook/logbook_columns en PG; se omite la verificación de conteos (solo aplica en bases con bitácora).');
        }
        ...
        console.log('⚙️  Secuencia id de limpieza_usuarios ajustada.');
    } catch (e) {
